...
The `require` directive tells the compiler that a module must be
loaded before the current module is compiled. In practice you only
reach for `require` when you want to use a module's *macros*, because
macros are expanded at compile time and therefore need the defining
module to already be there.

The most common example is Elixir's built-in `Logger`:
...
...>
...>   def warn_once do
...>     Logger.warning("something smells off") <2>
...>   end
...> end

iex> MyApp.warn_once()
16:42:15.001 [warning] something smells off
:ok
----
<1> `require Logger` makes the macros in the `Logger` module available
inside `MyApp`.
<2> `Logger.warning/1` is a macro, not a regular function. Without the
`require`, the compiler would complain.

NOTE: For regular (non-macro) functions you do not need `require`.
Reach for it only when the module's documentation tells you to or
when the compiler asks you to.
...
Once you've written a real module, you've also written the
territory where idiomatic refactoring lives. Claude Code
ships a built-in `/simplify` slash command that rewrites
the current file into more idiomatic Elixir while
preserving behaviour.

Where it's good in Elixir:

* Collapsing nested `case` / `if` chains into `with` or
  multi-clause functions.
* Turning explicit accumulators into a pipe (`|>`) through
  `Enum.map` / `Enum.reduce` / `Enum.filter`.
* Replacing `Map.get(m, :key, default)` chains with
  pattern matching on the key.
* Removing unnecessary intermediate variables when a pipe
  reads clearer.

Where it's not:

* Anything that affects the module's public contract
  (function names, arities, guards) — other code in the
  project may depend on the exact shape.
* Performance-sensitive code where `Enum` would allocate
  an intermediate list and `Stream` or a recursive
  function is faster.
* Code where an explicit, verbose form is the teaching
  point.

Treat `/simplify` as a "one file, one diff" tool. Always
read the diff before accepting; shorter code is not
