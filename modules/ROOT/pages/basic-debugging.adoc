...
with the xref:operators/pipe-operator.adoc[pipe operator] (`|>`), because
...
value, so it is optional. We cover xref:modules-and-functions.adoc#function-arity[arity]
...
When an AI agent adds a debug trace to a function, it
reaches for `IO.puts` because that's what Hello World
tutorials use. That is fine for a string literal and
nothing else. The moment the value is a map, tuple, list,
atom or `nil`, `IO.puts` throws away half the information:
...
`IO.inspect/2` prints the Elixir representation (what you
would type to recreate the value) and returns the value
itself, which means you can drop it into a pipe without
breaking the chain:
...
The `label:` option is the small detail that pays off
when you have more than one `IO.inspect` in the same
function. For richer output that includes the expression
and location, reach for `dbg/2` shown at the top of this
chapter.
