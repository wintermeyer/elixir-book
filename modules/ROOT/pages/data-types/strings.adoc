...
NOTE: Elixir also has a second text shape called a *charlist*, written
between single quotes. You only need it for some Erlang libraries. The
xref:binaries-and-charlists.adoc[Binaries and Charlists chapter] covers
the distinction.
...
BTW: There is no `join` function in the `String` module. But you can use `Enum.join/2` (from the xref:enumerables.adoc[`Enum` module], covered in a later chapter) for that:
